package com.rus.testingjava.domain;


import com.rus.testingjava.domain.utils.DotException;
import com.rus.testingjava.domain.utils.EmptyException;
import com.rus.testingjava.domain.utils.LettersException;
import com.rus.testingjava.domain.utils.NegativeException;
import com.rus.testingjava.domain.utils.NullException;
import com.rus.testingjava.domain.utils.SymbolsException;
import com.rus.testingjava.domain.utils.ZeroException;
    ...
    public String mul (String first, String second) {
        ...
        try {
            if (first == null || second == null) {
                ...
                throw new ZeroException("На 0 умножать нельзя!");
            } else if (patternRu.matcher(first).find() || patternRu.matcher(second).find()) {
                throw new LettersException("Отправка букв");
            } else if (patternEn.matcher(first).find() || patternEn.matcher(second).find()) {
                throw new LettersException("Отправка букв");
